...
public class UValidador {
	...
	private static final Log LOGGER = LogFactory.getLog(UValidador.class);
	...
	public static Character validarFlagLogicoChar(Boolean condicion) {
		if (condicion == null)
			return 'N';
		if (condicion)
			return 'S';
	...
	/**
	 * Retorna FALSE cuando el texto es igual a NULL o N o UN valor distinto a
	 * S. retorna TRUE solo en caso se pase como parametro S;
	 * 
	 * @param valor
	 *            texto que se evaluara, tipo String
	 * @return TRUE o FALSE dependiendo de la condicion cumplida
	...
	public static String nvlString(String valor, String reemplazo) {
		if (valor == null)
			return reemplazo;
		else
			return valor;
	...
	/**
	 * Retorna FALSE cuando todos los valores de la colección son falsos;
	 * 
	 * @param Lista
	 *            de valores lógicos, tipo Boolean.
	 * @return TRUE o FALSE dependiendo de la condición cumplida.
	 */
	public static Boolean obtenerValorDisjuntivo(List<Boolean> valores) {
		if (UValidador.esListaVacia(valores))
			return Boolean.FALSE;

		for (Boolean val : valores) {
			if (val) {
				return Boolean.TRUE;
			}
		}
	...
	public static String random(Integer cantidigt) {
		String result = "";
		while (cantidigt > 0) {
			result = result + "" + ((int) (Math.random() * 9 + 0)) + "";
			cantidigt = cantidigt - 1;
		}
		return result;
	}
