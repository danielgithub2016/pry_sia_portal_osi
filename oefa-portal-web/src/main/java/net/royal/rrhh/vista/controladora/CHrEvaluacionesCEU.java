...
@SessionScoped
@ManagedBean
public class CHrEvaluacionesCEU extends CBaseBean implements UIMantenimientoController {
	...
	private List<DtoHrEntrCasPostEval> lstIngresosSala;

	public CHrEvaluacionesCEU() {
		this.PANTALLA_LISTADO = "evaluacionListaCEU";
		this.PANTALLA_LISTADO_SALA = "evaluacionSalaCEU";
		this.PANTALLA_POSTULANTES = "postulantesListaEvalCEU";
		this.PANTALLA_POSTULANTES_SALA = "postulantesSalaCEU";
	}
	...
	public String verDetallesEvaluacion() {
		...
		if (!UValidador.esNulo(postulanteSeleccionado)) {
			lstIngresosSala = getFacRrhh().getHrEncuestaServicio().listarSession("EC",
					postulanteSeleccionado.getTipodocumento(), postulanteSeleccionado.getDocumento(),
	...
	public static CHrEvaluacionesCEU getInstance() {
		return UFaces.evaluateExpressionGet("#{cHrEvaluacionesCEU}", CHrEvaluacionesCEU.class);
	}
	...
	public String buscar() throws Exception {
		...
		List<DtoHrEvalCas> lstDatosFull = this.getFacRrhh().getHrEncuestaServicio()
				.listarHrEvalCeu(getUsuarioActual().getIdPersona());
	...
	public String buscarSalas() throws Exception {
		lstDatos = new ArrayList<DtoHrEvalCas>();
		LOGGER.debug(this.getUsuarioActual().getIdPersona());
		List<DtoHrEvalCas> lstDatosFull = this.getFacRrhh().getHrEncuestaServicio().listarHrEvalSalaCeu(convocatoria,
	...
	public String buscarParam() throws Exception {
		...
		lstDatos = new ArrayList<DtoHrEvalCas>();
		List<DtoHrEvalCas> lstDatosFull = this.getFacRrhh().getHrEncuestaServicio().listarHrEvalCeu2(convocatoria,
				despuesto, getUsuarioActual().getIdPersona());
		if (UValidador.esNulo(lstDatosFull))
			lstDatosFull = new ArrayList<DtoHrEvalCas>();
		LOGGER.debug("LISTA DE ENCUESTA " + lstDatosFull.size());
	...
	public String iniciar() throws Exception {
		this.setAccionPantalla(accion_solicitada.EDITAR);
		List<DtoHrEvalCas> lstDatosFull = this.getFacRrhh().getHrEncuestaServicio()
				.valInicioHrEvalCeu(registroSeleccionado);
		String val = "";
		for (int i = 0; i < lstDatosFull.size(); i++) {
		...
		if (val.equals("0")) {
			...
			this.getFacRrhh().getHrEncuestaServicio().editarHrEvalCeu(registroSeleccionado);
			this.buscar();
	...
	public String finalizar() throws Exception {
		...
		this.setAccionPantalla(accion_solicitada.EDITAR);
		this.getFacRrhh().getHrEncuestaServicio().finalizarHrEvalCeu(registroSeleccionado.getRequerimiento(),
				registroSeleccionado.getId_evalcas());
		String val = "";
	...
	public void listarPostulantes() {
		lstPost = this.getFacRrhh().getHrEncuestaServicio().verPostulantesHrEvalCeu(registroSeleccionado);
	}
	...
	public String listarPostulantesSala() {
		...
		if ("1".equals(registroSeleccionado.getN_estado())) {
			lstPostSala = this.getFacRrhh().getHrEncuestaServicio()
					.verPostulantesSalaHrEvalCeu(registroSeleccionadoSala, blFoto, null, documento);
		}
		if ("2".equals(registroSeleccionado.getN_estado())) {
			lstPostSala = this.getFacRrhh().getHrEncuestaServicio()
					.verPostulantesSalaHrEvalCeu2(registroSeleccionadoSala, blFoto, null, documento);
		}
	...
	public void listarPostulantes2() {
		lstPost = this.getFacRrhh().getHrEncuestaServicio().verPostulantesHrEvalCeu2(registroSeleccionado);
	}
	...
	public List<DtoHrEntrCasPostEval> getLstIngresosSala() {
		return lstIngresosSala;
	}

	public void setLstIngresosSala(List<DtoHrEntrCasPostEval> lstIngresosSala) {
		this.lstIngresosSala = lstIngresosSala;
	}
