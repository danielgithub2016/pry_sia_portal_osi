...
@ManagedBean
@SessionScoped
public class CwAsAutorizacionPreprocesoAprobacion extends CBaseBean implements UIMantenimientoController {
	...
	public String iniciarControladora() throws Exception {
		...
		//
		// emp = (Empleadomast)
		// getFacCore().getEmpleadomastServicio().obtenerPorId(emp.getPk());
		...
		//
		// area = (AsArea)
		// getFacAsistencia().getAsAreaServicio().obtenerPorId(area.getPk());
		//
		// if (!UValidador.esNulo(area)) {
		// // 1. si el empleado pertence a una area que no tiene
		// // superior
		// // puede procesar papeletas propias
		// if (UValidador.esCero(area.getAreapadre()) &&
		// (empleado.equals(area.getEmpleadoresponsable())
		...
		LOGGER.debug("Paso 10:wasautorizacionpreprocesoaprobacion");
	...
	private String edicionAutorizacion() throws Exception {
		...
		CwAsAutorizacionPreprocesoHextraEdit.getInstance().mensaje(mensajeGenerico);
	...
	public String exportar() throws Exception {
		...
		rowhead.createCell(3).setCellValue("Fecha ingreso");
		rowhead.createCell(4).setCellValue("Concepto");
		rowhead.createCell(5).setCellValue("Inicio");
		rowhead.createCell(6).setCellValue("Fin");
		rowhead.createCell(7).setCellValue("Desde");
		rowhead.createCell(8).setCellValue("Hasta");
		rowhead.createCell(9).setCellValue("Solicitado por");
		rowhead.createCell(10).setCellValue("Fecha solicitud");
		rowhead.createCell(11).setCellValue("Autorizado por");
		rowhead.createCell(12).setCellValue("Fecha autorización");
		rowhead.createCell(13).setCellValue("Estado");
		rowhead.createCell(14).setCellValue("Comentario");
		...
		for (DtoDwAsAutorizacionPreprocesoHextra obj : dtExportar) {
			...
			row.createCell(2).setCellValue(obj.getNombrecompleto());
			row.createCell(3).setCellValue(obj.getFechaingreso());
			row.createCell(4).setCellValue(obj.getConceptoaccesonombre());
			row.createCell(5).setCellValue(UFechaHora.convertirFechaCadena(obj.getFecha(), "dd/MM/yyyy"));
			row.createCell(6).setCellValue(UFechaHora.convertirFechaCadena(obj.getFechafin(), "dd/MM/yyyy"));
			row.createCell(7).setCellValue(UFechaHora.obtenerFechaParte(obj.getDesde1(), "HH") + ":"
					+ UFechaHora.obtenerFechaParte(obj.getDesde1(), "MI"));
			row.createCell(8).setCellValue(UFechaHora.obtenerFechaParte(obj.getHasta1(), "HH") + ":"
					+ UFechaHora.obtenerFechaParte(obj.getHasta1(), "MI"));
			row.createCell(9).setCellValue(obj.getSolicitadoPor());
			row.createCell(10).setCellValue(UFechaHora.convertirFechaCadena(obj.getFechaSolicitud(), "dd/MM/yyyy"));
			row.createCell(11).setCellValue(obj.getAutorizadoPor());
			row.createCell(12).setCellValue(obj.getFechaAutorizado() == null ? ""
	...
	public String guardar() throws Exception {
		...
		for (DtoDwAsAutorizacionPreprocesoHextra obj : lstAutorizacionPreProcesoAprobacion) {
			...
			if (obj.getIsseleccionaprobado() || obj.getIsseleccionrechazado()) {
				...
				if (!getFacSeguridad().getSeguridadautorizacionesServicio().tieneAccesoSysAdm(
						getUsuarioActual().getAplicacionCodigo(), getUsuarioActual().getCodigoUsuario())) {
					if ((!"S".equals(objconAux.getFlagAprobacionURH()) && !"S".equals(objconAux.getFlagaprobacionsst()))
							|| "S".equals(obj.getFlagAprobadoJefe())) {
						...
						if (UValidador.esCero(puedeAprobar)) {
							setMessageError(
									"Línea " + n + " Las solicitud por concepto de " + objconAux.getDescripcionlocal()
											+ " ya fué aprobada por el jefe, debe esperar la aprobación de URH.");
							return null;
						}
					}
				}
		...
		for (DtoDwAsAutorizacionPreprocesoHextra objaut : lstAutorizacionPreProcesoAprobacion) {
			...
			if (objaut.getIsseleccionaprobado() && objaut.getEstado().equals("S")
					&& (UConstante.CONSTANTE_CONCEPTO_RESOLUCION_0044.equalsIgnoreCase(objaut.getConceptoacceso())
							|| objaut.getConceptoacceso().equals(UConstante.CONSTANTE_CONCEPTO_LICENCIA_EXAMEN_ONCO))
	...
	private void guadarActivacionTemporalSIGED(AsAutorizacion dw_1) {

		AsDetallesigedPk pk = new AsDetallesigedPk();
		pk.setConceptoacceso(dw_1.getPk().getConceptoacceso());
		pk.setEmpleado(dw_1.getPk().getEmpleado());
		pk.setFechadesde(dw_1.getPk().getDesde());
		pk.setFechahasta(dw_1.getPk().getFecha());
		...
		for (AsDetallesiged bandeja : detalleRegistrado) {
			...
			asDetallesiged.getPk().setConceptoacceso(dw_1.getPk().getConceptoacceso());
			asDetallesiged.getPk().setEmpleado(dw_1.getPk().getEmpleado());
			asDetallesiged.getPk().setFechadesde(UFechaHora.anadirDiasAFechas(new Date(), -1));
			asDetallesiged.getPk().setFechahasta(UFechaHora.anadirDiasAFechas(new Date(), 1));
			asDetallesiged.getPk().setSecuencia(new BigDecimal(linea));
			asDetallesiged.setIdcargo(bandeja.getIdcargo());
			asDetallesiged.setIdarea(bandeja.getIdarea());
			asDetallesiged.setIdpersona(bandeja.getIdpersona());
			asDetallesiged.setEstado("A");
			asDetallesiged.setIdempleadosia(bandeja.getIdempleadosia());
	...
	public void procesoAprobacion() throws Exception {
		...
		for (DtoDwAsAutorizacionPreprocesoHextra obj : lstAutorizacionPreProcesoAprobacion) {
			...
			if (!UValidador.esNulo(findobj)) {
				if (("S".equals(findobj.getFlagAprobacionURH()) || "S".equals(findobj.getFlagaprobacionsst()))
						&& !"S".equals(obj.getFlagAprobadoJefe()) && "S".equals(w_Aprobar)) {
					w_AprobarJefe = "S";
					w_Aprobar = "N";
					w_Rechazar = "N";
				}
				...
				if (w_Aprobar.equals("S") || w_Rechazar.equals("S") || w_AprobarJefe.equals("S")) {
					...
					if (w_estado.equals("S")) {
						if (w_Aprobar.equals("S")) {
							...
							if (UConstante.CONSTANTE_CONCEPTO_RESOLUCION_0044.equalsIgnoreCase(obj.getConceptoacceso())
									|| UConstante.CONSTANTE_CONCEPTO_LICENCIA_EXAMEN_ONCO
							...
							if (!UValidador.esNulo(objupd)) {
								...
								// TODO SIGED si es Activacion Temporal eliminar
								// el Registro y guardarlos con las nuevas
								// fechas
								if (UConstante.CONSTANTE_CONCEPTO_ACTIVACION_TEMPORAL
										.equalsIgnoreCase(obj.getConceptoacceso())) {

									getFacAsistencia().getAsAutorizacionServicio().eliminar(objupd);

									// INSERTAR AUTORIZACION
									AsAutorizacion objins = new AsAutorizacion();
									objins.getPk().setConceptoacceso(objupd.getPk().getConceptoacceso());
									objins.getPk().setDesde(UFechaHora.anadirDiasAFechas(new Date(), -1));
									objins.getPk().setFecha(UFechaHora.anadirDiasAFechas(new Date(), 1));
									objins.getPk().setEmpleado(objupd.getPk().getEmpleado());
									objins.setHasta(obj.getHasta1());
									objins.setRefrigerio(obj.getRefrigerio());
									objins.setPeriodo(obj.getPeriodo());
									objins.setMandatorio(obj.getMandatorio());
									objins.setObservacion(obj.getObservacion());
									objins.setSolicitadopor(obj.getSolicitadoPor());
									objins.setFechasolicitud(obj.getFechaSolicitud());
									objins.setUltimoUsuario(obj.getUltimoUsuario());
									objins.setUltimaFechaModif(new Date());
									objins.setSobretiempoposicion("1");
									objins.setEstado(w_estadosiguiente);
									objins.setCategoriaautorizacion(obj.getCategoriaAutorizacion());
									objins.setTipohorario(obj.getTipohorario());
									objins.setRutadocumento(obj.getRutadocumento());
									objins.setTipoautorizacion(obj.getTipoautorizacion());
									objins.setNroresolucion(obj.getNroresolucion());
									objins.setFecharesolucion(obj.getFecharesolucion());
									objins.setDesderecupera(obj.getDesderecupera());
									objins.setHastarecupera(obj.getHastarecupera());
									objins.setPeriodoplanilla(obj.getPeriodoplanilla());

									getFacAsistencia().getAsAutorizacionServicio().registrar(objins);

									guadarActivacionTemporalSIGED(objupd);
	...
	private String enviarCorreo(String accion, DtoDwAsAutorizacionPreprocesoHextra obj) throws Exception {
		...
		if (UValidador.esCero(diasresta - 1)) {
			if (obj.getConceptoacceso().equals("OMIS")) {
				...
				// si es un concepto por dia no deberia salir la hora
				if ("H".equals(objcon.getExpresadoen())) {
		...
		// mensaje adicional;
		if ((obj.getConceptoacceso().equals("34")
				|| obj.getConceptoacceso().equals(UConstante.CONSTANTE_CONCEPTO_LICENCIA_EXAMEN_ONCO))
				&& accion.equals("A")) {
		...
		if (obj.getConceptoacceso().equals(UConstante.CONSTANTE_CONCEPTO_LICENCIA_EXAMEN_ONCO) && accion.equals("A")) {
	...
	public String verDocumento() throws Exception {
		...
		if (!UValidador.esNulo(uuidAlfresco)) {
			...
			case ConstanteSeguridad.PARAMETRO_MODOARCHIVO_ALFRESCO:
				...
				try {
					ContenidoECMBean respuestaconsulta = getuOsinergminAlfresco().obtenerDocumento(uuidAlfresco);
	...
	public static CwAsAutorizacionPreprocesoAprobacion getInstance() {
		return UFaces.evaluateExpressionGet("#{cwAsAutorizacionPreprocesoAprobacion}",
				CwAsAutorizacionPreprocesoAprobacion.class);
	}
