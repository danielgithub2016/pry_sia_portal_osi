...
import net.royal.asistencia.vista.paginacion.PWAsReporteCompensacion;
...
@ManagedBean
@SessionScoped
public class CwAsReporteCompensacionHoras extends CBaseBean implements UIMantenimientoController {
	...
	private List<DtoEmpleadomast> dw_2;
	...
	// Pagination List para la consulta más rápida
	private LazyDataModel<DtoEmpleadomast> lstCompensacion;
	...
	public String iniciarControladora() throws Exception {
		...
		return "waslistacompensacionhoras";
	}
	...
	public String buscar() throws Exception {
		...
		if (!UValidador.estaVacio(filtroPrep.getCodAreaOperativa())) {
			parametros.add(new ParametroPersistenciaGenerico("p_area", String.class, filtroPrep.getCodAreaOperativa()));
		}

		if (!UValidador.estaVacio(datosgenerico.getCodEmpleado())) {
			parametros
					.add(new ParametroPersistenciaGenerico("p_empleado", String.class, datosgenerico.getCodEmpleado()));
		}

		// dw_2 =
		// getFacCore().getEmpleadomastServicio().listarActivosArea(parametros);

		lstCompensacion = new PWAsReporteCompensacion(parametros, dwFiltros, getUsuarioActual());
	...
	public String exportar() throws Exception {
		...
		rowhead.createCell(0).setCellValue("Cod.");
		rowhead.createCell(1).setCellValue("Nombre");
		rowhead.createCell(2).setCellValue("Área Operativa");
		rowhead.createCell(3).setCellValue("Horas por Área");
		rowhead.createCell(4).setCellValue("Horas Generales");
		rowhead.createCell(5).setCellValue("Horas Consumidas");
		rowhead.createCell(6).setCellValue("Horas Solicitadas");
		rowhead.createCell(7).setCellValue("Semestre");
		...
		dw_2 = getFacCore().getEmpleadomastServicio().listarActivosArea(parametros);
		...
		if (!UValidador.esListaVacia(dw_2)) {
			...
			for (int i = 0; i < dw_2.size(); i++) {
				...
				DtoEmpleadomast obj = dw_2.get(i);
				// -----------------------------ACT HOCR 20180223
				// ----------------------------------//
				...
				if ("S".equals(area.getFlagexondirectiva())) {
					...
					ll_minutarthhee2 = ll_minutarthhee2.divide(new BigDecimal(60.0), 2, BigDecimal.ROUND_HALF_UP);
					...
					if (ls_FlagNumerosRestriccion.equals("S")) {
						...
						if (ll_CantidadMesesRestriccion.intValue() > 0) {
							...
							Empleadomast emp = (Empleadomast) getFacCore().getEmpleadomastServicio()
									.obtenerPorId(new EmpleadomastPk(obj.getEmpleado().intValue(),
											getUsuarioActual().getCompaniaSocioCodigo()));
							...
							ll_CantidadMesesRestriccionCAS = getFacAsistencia().getAsAutorizacionServicio()
									.obtenerHoraGanadas(obj.getEmpleado().intValue(), ld_fechainiciosemestre,
											ld_fechafinsemestre);
							// ll_minutarthhee =
							// getFacAsistencia().getAsAutorizacionServicio().obtenerMinutosSobreTiempo(
							...
							ll_minutarthhee2 = getFacAsistencia().getAsAutorizacionServicio().obtenerHorasSobreTiempo3(
									obj.getEmpleado().intValue(), ld_fechainiciosemestre, ld_fechafinsemestre);
				...
				row.createCell(0).setCellValue(obj.getEmpleado().toString());
				row.createCell(1).setCellValue(obj.getNombrecompleto());
				row.createCell(2).setCellValue(obj.getArea());
				row.createCell(3).setCellValue(obj.getHorasArea().toString());
				row.createCell(4).setCellValue(obj.getHorasGeneral().toString());
				row.createCell(5).setCellValue(obj.getHorasConsumidasTexto());
				row.createCell(6).setCellValue(obj.getHorasSolicitadasTexto());
				row.createCell(7).setCellValue(obj.getSemestre());
		...
		setSessionValue("nombrearchivo", "HorasConsumidas.xls");
	...
	public String editar() throws Exception {
		...
		if (registroSeleccionado.getEstado().equals("S") && registroSeleccionado.getFlagAprobadoJefe().equals("S")) {
	...
	public String guardar() throws Exception {
		...
		for (DtoDwAsAutorizacionPreprocesoHextra obj : lstAutorizacionPreProcesoAprobacion) {
			...
			if (obj.getIsseleccionaprobado() || obj.getIsseleccionrechazado()) {
				...
				if (UConstante.CONSTANTE_CONCEPTO_RESOLUCION_0044.equals(w_concepto)) {
					if (!isperfilas09) {
						...
						setMessageError("Línea " + n + ". Para procesar las solicitudes por concepto de "
								+ objconAux.getDescripcionlocal() + " debe tener asigando el perfil "
								+ objUsu.getNombre());
						return null;
					}
				} else {
								...
								if (obj.getFlagreprogramado().equals("S")) {
									...
									List<PrVacacionesprogramadas> programadas = getFacPlanilla()
											.getPrVacacionesprogramadasServicio().listarVacacionesProgramadas(
													objdelete.getPk().getEmpleado().intValue(), objdelete.getPeriodo(),
													objdelete.getPk().getFecha(), objdelete.getFechafin());
	...
	public static CwAsReporteCompensacionHoras getInstance() {
		return UFaces.evaluateExpressionGet("#{cwAsReporteCompensacionHoras}", CwAsReporteCompensacionHoras.class);
	}
	...
	public List<DtoEmpleadomast> getDw_2() {
		return dw_2;
	}

	public void setDw_2(List<DtoEmpleadomast> dw_2) {
		this.dw_2 = dw_2;
	}

	public LazyDataModel<DtoEmpleadomast> getLstCompensacion() {
		return lstCompensacion;
	}

	public void setLstCompensacion(LazyDataModel<DtoEmpleadomast> lstCompensacion) {
		this.lstCompensacion = lstCompensacion;
	}
